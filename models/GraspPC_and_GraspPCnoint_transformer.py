...
class PCTransformer(nn.Module):
    ...
    def __init__(self, in_chans=3, embed_dim=768, depth=[6, 6], num_heads=6, mlp_ratio=2., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
        ...
        #4 different coarse predictions
        self.coarse_pred = nn.Sequential(
            nn.Linear(1024, 1024),
            nn.ReLU(inplace=True),
            nn.Linear(1024, 3 * num_query)
        )
        self.coarse_pred2 = nn.Sequential(
            nn.Linear(1024, 1024),
            nn.ReLU(inplace=True),
            nn.Linear(1024, 3 * num_query)
        )
        self.coarse_pred3 = nn.Sequential(
            nn.Linear(1024, 1024),
            nn.ReLU(inplace=True),
            nn.Linear(1024, 3 * num_query)
        )
        self.coarse_pred4 = nn.Sequential(
            nn.Linear(1024, 1024),
            nn.ReLU(inplace=True),
            nn.Linear(1024, 3 * num_query)
        )
        #4 different mlp_queries 
    ...
    def forward(self, inpc):
        '''
            inpc : input incomplete point cloud with shape B N(1644) C(3)
        '''
        ...
        #NOTE: Coarse 1
        coarse_point_cloud1 = self.coarse_pred(global_feature).reshape(bs, -1, 3)  
        new_knn_index1 = get_knn_index(coarse_point_cloud1.transpose(1, 2).contiguous())
        cross_knn_index1 = get_knn_index(coor_k=coor, coor_q=coarse_point_cloud1.transpose(1, 2).contiguous())

        query_feature1 = torch.cat([
            global_feature.unsqueeze(1).expand(-1, self.num_query, -1), 
            coarse_point_cloud1], dim=-1) # B M C+3 
        #NOTE: Coarse 2
        coarse_point_cloud2 = self.coarse_pred2(global_feature).reshape(bs, -1, 3) 
        new_knn_index2 = get_knn_index(coarse_point_cloud2.transpose(1, 2).contiguous())
        cross_knn_index2 = get_knn_index(coor_k=coor, coor_q=coarse_point_cloud2.transpose(1, 2).contiguous())

        query_feature2 = torch.cat([
            global_feature.unsqueeze(1).expand(-1, self.num_query, -1), 
            coarse_point_cloud2], dim=-1) # B M C+3
        # #NOTE: Coarse 3
        coarse_point_cloud3 = self.coarse_pred3(global_feature).reshape(bs, -1, 3)  
        new_knn_index3 = get_knn_index(coarse_point_cloud3.transpose(1, 2).contiguous())
        cross_knn_index3 = get_knn_index(coor_k=coor, coor_q=coarse_point_cloud3.transpose(1, 2).contiguous())

        query_feature3 = torch.cat([
            global_feature.unsqueeze(1).expand(-1, self.num_query, -1), 
            coarse_point_cloud3], dim=-1) # B M C+3
        # #NOTE: Coarse 4
        coarse_point_cloud4 = self.coarse_pred4(global_feature).reshape(bs, -1, 3) 
        new_knn_index4 = get_knn_index(coarse_point_cloud4.transpose(1, 2).contiguous())
        cross_knn_index4 = get_knn_index(coor_k=coor, coor_q=coarse_point_cloud4.transpose(1, 2).contiguous())

        query_feature4 = torch.cat([
            global_feature.unsqueeze(1).expand(-1, self.num_query, -1), 
            coarse_point_cloud4], dim=-1)

        
        #4 different qs: q1-q4
        q = self.mlp_query(query_feature1.transpose(1,2)).transpose(1,2) 
        q2 = self.mlp_query2(query_feature2.transpose(1,2)).transpose(1,2) 
        q3 = self.mlp_query3(query_feature3.transpose(1,2)).transpose(1,2) 
        q4 = self.mlp_query4(query_feature4.transpose(1,2)).transpose(1,2) 
        
        # decoder
        for i, blk in enumerate(self.decoder):
            if i < self.knn_layer:
                q = blk(q, x, new_knn_index1, cross_knn_index1) 
                q2 = blk(q2, x, new_knn_index2, cross_knn_index2) 
                q3 = blk(q3, x, new_knn_index3, cross_knn_index3) 
                q4 = blk(q4, x, new_knn_index4, cross_knn_index4)   
            else:
                q = blk(q, x)
                q2 = blk(q2, x)
                q3 = blk(q3, x)
        ...
        return q, q2, q3, q4, coarse_point_cloud1, coarse_point_cloud2, coarse_point_cloud3, coarse_point_cloud4
