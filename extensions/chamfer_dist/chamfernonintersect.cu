...
__global__ void nonintersect_kernel(int batch_size,
                                    int n,
                                    const float* xyz1,
                                    int m,
                                    const float* xyz2,
  ...
  const int batch = 512;
  __shared__ float buf[batch * 3];
  __shared__ float nbuf[batch * 3];
  for (int i = blockIdx.x; i < batch_size; i += gridDim.x) {
    for (int k2 = 0; k2 < m; k2 += batch) {
      int end_k = min(m, k2 + batch) - k2;
      for (int j = threadIdx.x; j < end_k * 3; j += blockDim.x) {
        buf[j] = xyz2[(i * m + k2) * 3 + j];
        nbuf[j] = nxyz2[(i * m + k2) * 3 + j];
      }
      __syncthreads();
      for (int j = threadIdx.x + blockIdx.y * blockDim.x; j < n;
           j += blockDim.x * gridDim.y) {
          ...
          for (int k = 0; k < batch; k += 4) {
            {
              ...
              float nx2   = nbuf[k * 3 + 0];
              float ny2   = nbuf[k * 3 + 1];
              float nz2   = nbuf[k * 3 + 2];

              float dp = (nx2 * x2 + ny2 * y2 + nz2 * z2);
            ...
            }
            {
              ...
              float nx2   = nbuf[k * 3 + 3];
              float ny2   = nbuf[k * 3 + 4];
              float nz2   = nbuf[k * 3 + 5];
            ...
            }
            {
              ...
              float nx2   = nbuf[k * 3 + 6];
              float ny2   = nbuf[k * 3 + 7];
              float nz2   = nbuf[k * 3 + 8];
            ...
            }
            {
              ...
              float z2   = buf[k * 3 + 11] - z1;
              float dist = x2 * x2 + y2 * y2 + z2 * z2;
              float nx2   = nbuf[k * 3 + 9];
              float ny2   = nbuf[k * 3 + 10];
              float nz2   = nbuf[k * 3 + 11];
        ...
        if (k2 == 0 || dists[(i * n + j)] > best_dist) {
          dists[(i * n + j)]   = best_dist;
          dps[(i * n + j)]     = best_dist_dp;
          indexes[(i * n + j)] = best_dist_index;
...
std::vector<torch::Tensor> nonintersect_cuda_forward(torch::Tensor xyz1,
  ...
  nonintersect_kernel<<<dim3(32, 16, 1), 512>>>(
    batch_size, n, xyz1.data_ptr<float>(), m, xyz2.data_ptr<float>(), nxyz2.data_ptr<float>(),
    dps1.data_ptr<float>(), dists1.data_ptr<float>(), idx1.data_ptr<int>());
  
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess) {
    printf("Error in nonintersect_cuda_forward: %s\n", cudaGetErrorString(err));
  }
  return {dps1, dists1, idx1};
}
...
__global__ void nonintersect_grad_kernel(int b,
                                         int n,
                                         const float* xyz1,
                                         int m,
                                         const float* xyz2,
                                         const float* nxyz2,
                                         const float* grad_dps1,
                                         const int* idx1,
                                         float* grad_xyz1,
                                         float* grad_xyz2,
                                         float* grad_nxyz2) {
  ...
  for (int i = blockIdx.x; i < b; i += gridDim.x) {
    for (int j = threadIdx.x + blockIdx.y * blockDim.x; j < n;
      ...
      float nx2 = nxyz2[(i * m + j2) * 3 + 0];
      float ny2 = nxyz2[(i * m + j2) * 3 + 1];
      float nz2 = nxyz2[(i * m + j2) * 3 + 2];
      
      float g  = grad_dps1[i * n + j];
      
      atomicAdd(&(grad_xyz1[(i * n + j) * 3 + 0]), -g * nx2); 
      atomicAdd(&(grad_xyz1[(i * n + j) * 3 + 1]), -g * ny2);
      atomicAdd(&(grad_xyz1[(i * n + j) * 3 + 2]), -g * nz2);
      
      atomicAdd(&(grad_xyz2[(i * m + j2) * 3 + 0]), g * nx2);
      atomicAdd(&(grad_xyz2[(i * m + j2) * 3 + 1]), g * ny2);
      atomicAdd(&(grad_xyz2[(i * m + j2) * 3 + 2]), g * nz2);

      atomicAdd(&(grad_nxyz2[(i * m + j2) * 3 + 0]), -(g * (x1 - x2)));
      atomicAdd(&(grad_nxyz2[(i * m + j2) * 3 + 1]), -(g * (y1 - y2)));
      atomicAdd(&(grad_nxyz2[(i * m + j2) * 3 + 2]), -(g * (z1 - z2)));
    }
  }
}
...
std::vector<torch::Tensor> nonintersect_cuda_backward(torch::Tensor xyz1,
  ...
  torch::Tensor grad_xyz1 = torch::zeros_like(xyz1, torch::CUDA(torch::kFloat));
  torch::Tensor grad_xyz2 = torch::zeros_like(xyz2, torch::CUDA(torch::kFloat));
  torch::Tensor grad_nxyz2 = torch::zeros_like(xyz2, torch::CUDA(torch::kFloat));
  
  nonintersect_grad_kernel<<<dim3(1, 16, 1), 256>>>(
    batch_size, n, xyz1.data_ptr<float>(), m, xyz2.data_ptr<float>(), nxyz2.data_ptr<float>(),
    grad_dps1.data_ptr<float>(), idx1.data_ptr<int>(),
    grad_xyz1.data_ptr<float>(), grad_xyz2.data_ptr<float>(), grad_nxyz2.data_ptr<float>());

  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess) {
    printf("Error in nonintersect_cuda_backward: %s\n", cudaGetErrorString(err));
  }
