...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (name, data_input, data_normals, data_output) in enumerate(train_dataloader):
            ...
            if  'PCN' in dataset_name or dataset_name == 'Completion3D' or 'ProjectShapeNet' or "HO3d" in dataset_name: #
...
def test(base_model, test_dataloader,ChamferDisL1, ChamferDisL2, args, config, logger = None):
    ...
    with torch.no_grad():
        for  idx, (name, data_input, data_normals, data_output) in enumerate(test_dataloader):
            ...
            if  'PCN' in dataset_name or 'ProjectShapeNet'or "HO3d" in dataset_name: 
                ...
                output1_file_path = f"GraspPC/output/{name}/output1_pred_{name}.ply"
                output2_file_path = f"GraspPC/output/{name}/output2_pred__{name}.ply"
                output3_file_path = f"GraspPC/output/{name}/output3_pred_{name}.ply"
                output4_file_path = f"GraspPC/output/{name}/output4_pred_{name}.ply"
