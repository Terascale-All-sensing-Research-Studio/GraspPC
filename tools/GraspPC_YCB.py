...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        #NOTE: TRAINING WAS NOT USED FOR YCB ONLY TEST, This training is just GraspPC_noint copy
        for idx, (taxonomy_ids, model_ids, obj_ID_used, data_input, data_output, data_normals) in enumerate(train_dataloader):
            ...
            if  'PCN' in dataset_name or dataset_name == 'Completion3D' or 'ProjectShapeNet' or "HOH_3dm" or in dataset_name: #
...
def test(base_model, test_dataloader,ChamferDisL1, ChamferDisL2, args, config, logger = None):
    ...
    with torch.no_grad():
        for idx, (name, data_input,number) in enumerate(test_dataloader):
            name = name[0]
            number = number[0]
            ...
            if  'PCN' in dataset_name or 'ProjectShapeNet'  or "YCB" in dataset_name: 
                input_object = data_input.cuda() if data_input is not None else None
                ...
                output_file = f"GraspPC/outputs/{name}/input_{name}_{number}.ply"
                ...
                output1_file_path = f"GraspPC/outputs/{name}/output1_pred_{name}_{number}.ply"
                output2_file_path = f"GraspPC/outputs/{name}/output2_pred_{name}_{number}.ply"
                output3_file_path = f"GraspPC/outputs/{name}/output3_pred_{name}_{number}.ply"
                output4_file_path = f"GraspPC/outputs/{name}/output4_pred_{name}_{number}.ply"
