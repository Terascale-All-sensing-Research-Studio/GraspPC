...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        # losses = AverageMeter(['SparseLoss', 'DenseLoss', "SparseLossObject", "DenseLossObject"])
        # losses = AverageMeter(['SparseLoss', 'DenseLoss', "SparseLossObject"])
        # losses = AverageMeter(['SparseLoss', 'DenseLoss', "DenseLossObject"])
        # losses = AverageMeter(['SparseLossO1', 'DenseLossO1','SparseLossO2', 'DenseLossO2'])
    ...
    with torch.no_grad():
        for  idx, (name, data_input, data_normals, data_output) in enumerate(test_dataloader):
            ...
            if  'PCN' in dataset_name or 'ProjectShapeNet'or "DexYCB" in dataset_name:
                ...
                output_file = f"GraspPC/outputs/{name}/input_{name}.ply"
                ...
                output1_file_path = f"GraspPC/outputs/{name}/output1_pred_{name}.ply"
                output2_file_path = f"GraspPC/outputs/{name}/output2_pred__{name}.ply"
                output3_file_path = f"GraspPC/outputs/{name}/output3_pred_{name}.ply"
                output4_file_path = f"GraspPC/outputs/{name}/output4_pred_{name}.ply"
