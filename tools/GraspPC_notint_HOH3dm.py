...
from extensions.chamfer_dist import  ChamferDistanceL1Coarse, ChamferDistanceL1Dense, ChamferDistanceL2, MinIndicesManager, ChamferDistanceL1, ChamferFunctionUni
import sys
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, obj_ID_used, data_input, data_output, data_normals) in enumerate(train_dataloader):
            ...
            if  'PCN' in dataset_name or dataset_name == 'Completion3D' or 'ProjectShapeNet' or "HOH_3dm"  in dataset_name: 
                ...
                threedm_normals = data_normals.cuda() if data_normals is not None else None
                ...
                if output_object is None or torch.all(output_object == 0):
                    continue
                if threedm_normals is None or torch.all(threedm_normals == 0):
                    continue
            ...
            #get the different losses 
            sparse_loss_output_1, dense_loss_output_1 = base_model.module.get_loss(ret, output_object, epoch)
            sparse_loss_output_2, dense_loss_output_2 = base_model.module.get_loss2(ret, output_object, epoch)
            sparse_loss_output_3, dense_loss_output_3 = base_model.module.get_loss3(ret, output_object, epoch)
            sparse_loss_output_4, dense_loss_output_4 = base_model.module.get_loss4(ret, output_object, epoch)

            
            penetration_loss1 = base_model.module.get_penetration_loss1(ret, input_object, threedm_normals, epoch)
            penetration_loss2 = base_model.module.get_penetration_loss2(ret, input_object, threedm_normals, epoch)
            penetration_loss3 = base_model.module.get_penetration_loss3(ret, input_object, threedm_normals, epoch)
            penetration_loss4 = base_model.module.get_penetration_loss4(ret, input_object, threedm_normals, epoch)
            

            
            #have to add another dimension to the losses to concat them into a 48x 3
            ...
            csv_name = "example"
            csv_file_path = f"GraspPC/losses/{csv_name}.csv"
...
def test(base_model, test_dataloader,ChamferDisL1, ChamferDisL2, args, config, logger = None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, obj_ID_used, data_input, data_output, data_normals) in enumerate(test_dataloader):
            ...
            if  'PCN' in dataset_name or 'ProjectShapeNet'or "HOH_3dm"  in dataset_name: #or "HOH_dataset"
                input_object = data_input.cuda() if data_input is not None else None
                output_object  = data_output.cuda() if data_output is not None else None
                normals  = data_normals.cuda() if data_output is not None else None
                ...
                coarse_points1 = ret[0][0]
                coarse_points2 = ret[0][1]
                coarse_points3 = ret[0][2]
                coarse_points4 = ret[0][3]
                ...
                #save input ptclds
                input_object_ptcld = input_object[0].cpu().numpy()  
                ...
                output_file = f"/GraspPC/CVPR_outputs/{name}/input_{taxonomy_id}_{model_id}_{obj_ID_used}.ply"
                input_object_ptcld.export(output_file)
              
                
                
                #save the predictions
                output1_dense_np = output1_dense[0].cpu().detach().numpy() 
                output2_dense_np = output2_dense[0].cpu().detach().numpy()  
                output3_dense_np = output3_dense[0].cpu().detach().numpy()  
                output4_dense_np = output4_dense[0].cpu().detach().numpy()  
                


                output1_file_path = f"GraspPC/CVPR_outputs/{name}/output1_pred_{taxonomy_id}_{model_id}_{obj_ID_used}.ply"
                output2_file_path = f"GraspPC/CVPR_outputs/{name}/output2_pred_{taxonomy_id}_{model_id}_{obj_ID_used}.ply"
                output3_file_path = f"GraspPC/CVPR_outputs/{name}/output3_pred_{taxonomy_id}_{model_id}_{obj_ID_used}.ply"
                output4_file_path = f"GraspPC/CVPR_outputs/{name}/output4_pred_{taxonomy_id}_{model_id}_{obj_ID_used}.ply"
                ...
                # prediction output 1 
                output1_coarse_ptcld = trimesh.PointCloud(output1_coarse_np)
                output1_coarse_ptcld.colors = np.tile(np.array([0, 255, 0]), (output1_coarse_ptcld.vertices.shape[0], 1))
                
                #prediction output 2
                output2_coarse_ptcld = trimesh.PointCloud(output2_coarse_np)
                output2_coarse_ptcld.colors = np.tile(np.array([255, 255, 255]), (output2_coarse_ptcld.vertices.shape[0], 1))
                
                output3_coarse_ptcld = trimesh.PointCloud(output3_coarse_np)
                output3_coarse_ptcld.colors = np.tile(np.array([225, 225, 127]), (output3_coarse_ptcld.vertices.shape[0], 1))
                
                output4_coarse_ptcld = trimesh.PointCloud(output4_coarse_np)
                output4_coarse_ptcld.colors = np.tile(np.array([170, 0, 0]), (output4_coarse_ptcld.vertices.shape[0], 1))
